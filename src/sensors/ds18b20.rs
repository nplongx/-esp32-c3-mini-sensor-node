...
impl<'a> HydroponicTempSensor<'a> {
    ...
    pub fn new(mut pin_driver: PinDriver<'a, InputOutput>) -> anyhow::Result<Self> {
        ...
        pin_driver
            .set_high()
            .map_err(|e| anyhow::anyhow!("Không thể set_high cho chân cảm biến: {:?}", e))?;

        let mut one_wire =
            OneWire::new(pin_driver).map_err(|_| anyhow::anyhow!("Lỗi khởi tạo OneWire"))?;
    ...
    pub fn read_temperature(&mut self) -> anyhow::Result<Option<f32>> {
        ...
        let sensor = Ds18b20::new::<()>(address)
            .map_err(|_| anyhow::anyhow!("Lỗi khởi tạo đối tượng DS18B20"))?;
